...
const promotionStateSummary = (ad: AdminUserNestedAd): string => {
  const top = ad.is_top_sale ? 'Top sotuv yoqilgan' : 'Top sotuv yo‘q'
  if (ad.is_boosted) {
    const end = ad.boost_expires_at ? formatUzbekDateTime(ad.boost_expires_at) : '—'
    return `${top} · Boost yoqilgan · tugash: ${end}`
  }
  return `${top} · Boost yo‘q`
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-10 md:items-stretch">
        ...
        <section className={`${cardClass} col-span-1 md:col-span-10`}>
          <h2 className="text-base font-semibold text-slate-900 dark:text-slate-100">Promo holati</h2>
          <p className="mt-2 text-sm leading-relaxed text-slate-700 dark:text-slate-200">
            {promotionStateSummary(ad)}
          </p>
          <p className="mt-3 text-sm text-slate-600 dark:text-slate-400">
            <Link to={promotionsHref} className="font-medium text-daladan-primary hover:underline">
              Reklama tarixini ko‘rish
            </Link>
          </p>
        </section>
      </div>
